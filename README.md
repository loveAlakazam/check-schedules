...
모든 스케줄이 겹치는 최악의 상황이 아니라면, 먼저 스케줄들을 정렬시킨뒤에 스케줄쌍을 비교하여 처리하는 방법입니다.
이는 Sweep Line 알고리즘 으로 정의되며, 시간복잡도가 `O(NlogN + N * K) (단, K<= N)` 입니다. 자세한 과정은 하단에 기재되어 있습니다.
...
> phase 2: index=0인 스케줄을 기준으로 할 때
